...
//        var colList = pingTasks.Where(p => p.Result.Status == IPStatus.Success).Select(p => p.Result.Address).ToList();

//        foreach (var p in colList)
//        {
...
//        var colList = devDescTasks.Where(t => t.Result.IsCanonCamera).Select(t => t.Result.CameraDevDesc!).ToList();

//        foreach (var d in colList)
//        {
